 ...
 *	
 *	Tool #1 is for image pre-processing.
 *	Tool #2, #3 are for image processing of nuclei channels.
 ...
 *	Tool #7 is for ROI FU quantification.
 *	
...
macro "Convert and/or split Action Tool - T06091T0f09CT5f09oTaf09nTff09v"  {
...
if (Question == 1) {
	...
	var file_extension = newArray("nd2", "czi", "Tif", "jpg", "png");

	Dialog.create("Image format");
		Dialog.addChoice("The file extension of your image is: ", file_extension);
		Dialog.show();
...
setBatchMode(true);
	...
	for (c = 1 ; c <= channels ; c++){
		selectImage(c);
//	currentImage_name = getTitle(); // Modification 31.07.25
	name_1 = getTitle(); // Modification 31.07.25
	currentImage_name = replace(name_1, ".tif", ""); // Modification 31.07.25
	...
	Message1 = "Several mono-channel images have been detected.";
...
macro "Normalisation Action Tool - T06092T0f09NT5f09oTaf09rTff09m"   {
	Title = "Normalisation";
...
macro "Background correction (Nuclei) Action Tool - T06093T0f09CT5f09.Taf09NTff09u"	{
	...
	Ball = getNumber("Max diameter estimated of your objects: ", 10);
...
macro "Background correction (Particles) Action Tool - T06094T0f09CT5f09.Taf09PTff09a" {
	...
	Ball = getNumber("Max diameter estimated of your particles: ", 1);
	Sigma1 = getNumber("Sigma 1: ", 1);
	Sigma2 = getNumber("Sigma 2: ", 5);
		print("Rolling ball radius: "+Ball);
...
macro "Counting (Nuclei) Action Tool - T06095T0f09CT5f09oTaf09uTff09n" {
	...
	continue;
		...
		setAutoThreshold("Otsu dark no-reset"); // Ajouter le 30.07.25		
		run("Threshold...");
	...
	Dialog.addMessage("Those parameters below can be user-defined.");
		...
		Dialog.addMessage("Surface of your object (Area)");
		Dialog.addSlider("Size [min]: ", min, max, 10);
	...
	maxCircularity = Dialog.getNumber();
		...
		print("Threshold minimum: "+Low_Threshold);
...
//		run("Watershed");
//}
//		run("Watershed"); Modifier le 30.07.25 (A enlever cette étape)
	...
	for (x = 0 ; x < Group; x++) {
		...
		roiManager("Set Color", 1 + x + "green");
}
		selectImage(currentImage_name);
		setThreshold(Low_Threshold, upper, "raw");
...
macro "Counting (Particles) Action Tool - T06096T0f09CT5f09oTaf09uTff09n" {
	...
	list = getFileList(input);
	output_dir = input + File.separator + "3 - Data "+channel_name+ File.separator;
		File.makeDirectory(output_dir);
	...
	continue;
		...
		getThreshold(lower, upper);
			...
			average = sum / (nImage-1);													// Calculate the average lower threshold value.
		print("Average lower threshold value through all images: " + average);
		print("The average lower threshold calculated will distinguish what is a true positive from what is a true negative staining.");
		print("It is going to be a constant for all of your images.");
		print("");
	...
	Dialog.addMessage("Those parameters below can be user-defined.");
		Dialog.addMessage("Average lowest threshold can be read from the log windows.");
		Dialog.addNumber("Lowest threshold: ", threshold);
		Dialog.addMessage("Surface of your particles (Area)");
		Dialog.addSlider("Size [min]: ", min, max, 0.5);
...
setBatchMode(true);
	...
	roi_name = Roi.getName;
		run("Analyze Particles...", "size="+minSize+"-"+maxSize+" circularity="+minCircularity+"-"+maxCircularity+"show=Masks exclude clear");
...
macro "Quantification FU Macro Action Tool - T06097T0f09QT5f09.Taf09FTff09U" {
	Title = "Region of interest (ROI) fluorescence quantification";
	Message = "Fluorescence quantification for each region of interest is about to begin.\nPress OK to continue."
		waitForUser(Title, Message);
...
//	currentImage_name = getTitle(); // Modification 31.07.25
	name_1 = getTitle(); // Modification 31.07.25
	currentImage_name = replace(name_1, ".tif", ""); // Modification 31.07.25
		run("Set Measurements...", "area mean standard modal min centroid center perimeter bounding fit shape feret's integrated median skewness kurtosis area_fraction limit display redirect=None decimal=4");
	showMessage("You are going to be asked to look for a ROI.zip folder.\nThe folder is located in the nuclei channel /3 - .... Datas/.\nNB: Read the log to know which one you need by reading the name.");
		print("Current image: "+currentImage_name+" ROI-nuclei.zip");
	ROI_folder = File.openDialog("Select the ROI folder to quantify fluorescence for each ROI.");
		roiManager("Open", ROI_folder);
		roiManager("Select", newArray());
	...
	if (ROI >=1 ) {
		...
		roiManager("Measure");
		selectWindow("Results");
		saveAs("Results", output_dir+currentImage_name+"-FU datas.tsv");
		run("Close");
		roiManager("delete");
		run("Close All");
}	else {
		print("No R0Is were found in: "+currentImage_name);
		run("Close All");
}
}
		selectWindow("Log");
	...
	dialog = Dialog.create("Question");
	Dialog.addMessage("Do you wish to quantify fluorescence from any others channels ?");
		Dialog.addChoice("Choose:", newArray("Yes", "No, I am done"));
		Dialog.show();
	choice = Dialog.getChoice();
	} while (choice == "Yes");
	Title3 = "Fluorescence quantification has ended";
	Message3 = "Press OK to Continue";
		showMessage(Title3, Message3);
}

/*  QuantLim  Copyright (C) 08.2023  Jean-Yves Alejandro Frayssinhes
